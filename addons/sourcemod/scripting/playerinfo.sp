...
public Plugin myinfo =
{
    ...
    author = "akz + lexzor",
    description = "Saves some information of each player into a database to integrate their account in GameServices platform.",
    version = "2.2",
...
{
    g_cvTableName = CreateConVar("sm_playerinfo_table", "players", "Name of the table where the SQL queries will be executed");
...
public void ConnectToDb(Database db, const char[] failure, any data)
{
    ...
    else
    {
        ...
        Format(query, sizeof(query),
            "CREATE TABLE IF NOT EXISTS %s (name VARCHAR(%i) NOT NULL,steamid VARCHAR(32) NOT NULL,platform_user_id BIGINT NULL DEFAULT NULL,address VARCHAR(45) NOT NULL,country VARCHAR(2) NOT NULL,first_join DATETIME NOT NULL,last_seen DATETIME DEFAULT CURRENT_TIMESTAMP,connections INT NOT NULL DEFAULT 0,time INT NOT NULL DEFAULT 0,PRIMARY KEY (steamid),INDEX idx_platform_user_id (platform_user_id))",
...
public void QuerySql(char[] name, char[] steamid, char[] address, char[] country, int seconds, int option)
{
    ...
    else
    {
        ...
        if (update)
        {
            Format(query, sizeof(query), "UPDATE %s SET name = '%s', last_seen = NOW(), connections = connections + 1, time = time + %i WHERE steamid = '%s'", table, name, seconds, steamid);
        }
        else
        {
            Format(query, sizeof(query), "UPDATE %s SET last_seen = NOW(), connections = connections + 1, time = time + %i WHERE steamid = '%s'", table, seconds, steamid);
        }
    }
    if (!SQL_FastQuery(g_dbDatabase, query))
    {
